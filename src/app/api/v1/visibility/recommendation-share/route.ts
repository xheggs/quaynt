...
import {
  ...
  RECOMMENDATION_SHARE_ALLOWED_SORTS,
} from '@/modules/visibility/recommendation-share.service';
    ...
    withAuth(
      withRateLimit(
        withScope(async (req) => {
          ...
          // Check for latest mode
          const latest = params.get('latest') === 'true';
          if (latest) {
            const items = await getLatestRecommendationShare(
              auth.workspaceId,
              promptSetId,
          ...
          const { items, total } = await getRecommendationShare(
            auth.workspaceId,
            filters,
            pagination
          );
