...
import { getDualHistory } from '@/modules/visibility/dual-score.service';
...
/**
 * GET /api/v1/visibility/dual-score/history
 * Aligned SEO + GEO snapshot pairs with per-snapshot deltas.
 */
    ...
    withAuth(
      withRateLimit(
        withScope(async (req) => {
          ...
          const result = await getDualHistory(auth.workspaceId, brandId, from, to, granularity);
