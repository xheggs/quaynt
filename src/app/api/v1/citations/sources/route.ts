...
} from '@/modules/visibility/citation-source-aggregate.service';
    ...
    withAuth(
      withRateLimit(
        withScope(async (req) => {
          ...
          if (!promptSetId) {
            return badRequest('A prompt set (market) is required to view citation sources');
          }
          ...
          const { items, total } = await getCitationSources(auth.workspaceId, filters, pagination);
