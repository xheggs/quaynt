...
const mockComputePositionAggregate = vi.fn();
...
vi.mock('./position-aggregate.compute', () => ({
  computePositionAggregate: (...args: unknown[]) => mockComputePositionAggregate(...args),
}));
...
describe('registerPositionAggregateHandlers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('registers compute and reconcile handlers with correct schedule', async () => {
    ...
    const { registerPositionAggregateHandlers } = await import('./position-aggregate.handler');
    await registerPositionAggregateHandlers(boss as unknown as PgBoss);
  ...
  describe('compute handler', () => {
    ...
    it('calls computePositionAggregate with correct args', async () => {
      mockComputePositionAggregate.mockResolvedValueOnce({ changed: false });
      ...
      expect(mockComputePositionAggregate).toHaveBeenCalledWith({
        workspaceId: 'ws_test',
        promptSetId: 'ps_test',
        date: '2026-04-03',
      });
    ...
    it('dispatches webhook when aggregates changed', async () => {
      mockComputePositionAggregate.mockResolvedValueOnce({ changed: true });
      ...
      expect(mockDispatchWebhookEvent).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          positionAggregate: expect.objectContaining({
            workspaceId: 'ws_test',
            promptSetId: 'ps_test',
            date: '2026-04-03',
          }),
    ...
    it('handles errors gracefully (re-throws for pg-boss retry)', async () => {
      mockComputePositionAggregate.mockRejectedValueOnce(new Error('DB error'));
      const handler = await getComputeHandler();
