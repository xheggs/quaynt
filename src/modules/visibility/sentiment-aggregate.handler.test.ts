...
const mockComputeSentimentAggregate = vi.fn();
...
vi.mock('./sentiment-aggregate.compute', () => ({
  computeSentimentAggregate: (...args: unknown[]) => mockComputeSentimentAggregate(...args),
}));
...
describe('registerSentimentAggregateHandlers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('registers compute and reconcile handlers with correct schedule', async () => {
    ...
    const { registerSentimentAggregateHandlers } = await import('./sentiment-aggregate.handler');
    await registerSentimentAggregateHandlers(boss as unknown as PgBoss);
  ...
  describe('compute handler', () => {
    ...
    it('calls computeSentimentAggregate with correct args', async () => {
      mockComputeSentimentAggregate.mockResolvedValueOnce({ changed: false });
      ...
      expect(mockComputeSentimentAggregate).toHaveBeenCalledWith({
        workspaceId: 'ws_test',
        promptSetId: 'ps_test',
        date: '2026-04-03',
      });
    ...
    it('dispatches webhook when aggregates changed', async () => {
      mockComputeSentimentAggregate.mockResolvedValueOnce({ changed: true });
      ...
      expect(mockDispatchWebhookEvent).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          sentimentAggregate: expect.objectContaining({
            workspaceId: 'ws_test',
            promptSetId: 'ps_test',
            date: '2026-04-03',
          }),
    ...
    it('handles errors gracefully (re-throws for pg-boss retry)', async () => {
      mockComputeSentimentAggregate.mockRejectedValueOnce(new Error('DB error'));
      const handler = await getComputeHandler();
