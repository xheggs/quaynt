...
import type { SeoScoreRecommendation } from '../seo-score.types';
            ...
            <CardContent className="space-y-2 py-4">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="secondary" className="text-xs">
                  {t(`factor.${rec.factorId}.name` as 'factor.impression_volume.name')}
                </Badge>
                <span
                  className="text-xs font-medium text-emerald-700 dark:text-emerald-400"
                  title={t('recommendations.estDeltaTooltip')}
                >
                  {t('recommendations.estDelta', {
                    delta: rec.estimatedPointDelta,
                  })}
                </span>
              </div>
            </CardContent>
